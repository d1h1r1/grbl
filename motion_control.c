...
void mc_line(float *target, plan_line_data_t *pl_data)
{
  ...
  do {
    ...
    if (sys.abort) { return; } // 如果系统中止，则退出。
  ...
  uint8_t axis_0, uint8_t axis_1, uint8_t axis_linear, uint8_t is_clockwise_arc)
{
  ...
  float angular_travel = atan2(r_axis0 * rt_axis1 - r_axis1 * rt_axis0, r_axis0 * r_axis0 + r_axis1 * rt_axis1);
  if (is_clockwise_arc) { // 根据方向修正 atan2 输出
  ...
  if (segments) {
    ...
    */
    ...
    for (i = 1; i < segments; i++) { // 增量（segments - 1）。
...
uint8_t mc_probe_cycle(float *target, plan_line_data_t *pl_data, uint8_t parser_flags)
{
  ...
  protocol_buffer_synchronize();
  if (sys.abort) { return(GC_PROBE_ABORT); } // 如果已经发出系统重置，则返回。
