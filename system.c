...
uint8_t system_execute_line(char *line)
{
  ...
  switch( line[char_counter] ) {
    ...
    case '$': case 'G': case 'C': case 'X':
      ...
      switch( line[1] ) {
        case '$' : // 打印 Grbl 设置
          if ( sys.state & (STATE_CYCLE | STATE_HOLD) ) { return(STATUS_IDLE_ERROR); } // 在循环期间阻止。打印时间过长。
        ...
        case 'C' : // 设置检查 g-code 模式 [IDLE/CHECK]
          ...
          if ( sys.state == STATE_CHECK_MODE ) {
            ...
            if (sys.state) { return(STATUS_IDLE_ERROR); } // 需要无警报模式。
    ...
    default :
      ...
      switch( line[1] ) {
        ...
        case 'H' : // 执行归位循环 [IDLE/ALARM]
          ...
          if (system_check_safety_door_ajar()) { return(STATUS_CHECK_DOOR); } // 如果安全门未关闭，则阻止。
